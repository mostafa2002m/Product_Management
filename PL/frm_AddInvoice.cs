...
namespace PRODUCTS_MANAGEMENT.PL
{
    public partial class frm_AddInvoice : Form
    {
        ...
        private void txtdiscount_KeyUp_1(object sender, KeyEventArgs e)
        {
            CalculateNetValue();
        ...
        private void btnSaveOrder_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Ins == true)
                {
                    ...
                    if (rdbCash.Checked)
                    {
                        Add_Payment_Cash("Repayment of purchases");
                        MessageBox.Show(@"Payment is made ");
                    }
                    //else
                    //{
                    //    Add_Payment_Credit("فاتورة مشتريات");
        ...
        private void insert_Invoice(bool INS)
        {
            try
            {
                int x = cls_Invoice.stp_Insert_Invoice(int.Parse(txtOrder_Id.Text), dtpOrder_Date.Value, Convert.ToDecimal(txtOrder_Total.Text), int.Parse(txtCust_Id.Text), txtOrder_SaleMan.Text, lblType.Text);
                if (x > 0)
                {
        ...
        private void cboCust_Name_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txtCust_Name.Text = dt.Rows[0]["Sup_Name"].ToString();
                //txtCust_Address.Text = dt.Rows[0]["Sup_Address"].ToString();
                //txtCust_Phone.Text = dt.Rows[0]["Sup_Phone"].ToString();
                //txtCust_Email.Text = dt.Rows[0]["Sup_Email"].ToString();
